            ...
            async success(ctx, response) {
                ...
                if (response.provider === "github") {
                    ...
                    if (!primaryEmail.verified)
                        throw new Error("Primary email for GitHub user not verified");
                    email = primaryEmail.email;
                }
                else if (response.provider === "google") {
                    if (!response.id.email_verified)
                        throw new Error("Google email not verified");
                    subject = response.id.sub;
                ...
                const accountID = await (async () => {
                    const matches = await Database.use(async (tx) => tx
                        .select({
                        provider: AuthTable.provider,
                        accountID: AuthTable.accountID,
                    })
                        .from(AuthTable)
                        .where(or(and(eq(AuthTable.provider, response.provider), eq(AuthTable.subject, subject)), and(eq(AuthTable.provider, "email"), eq(AuthTable.subject, email)))));
                    ...
                    await Database.use(async (tx) => tx
                        .insert(AuthTable)
                        .values([
                        {
                ...
                await Actor.provide("account", { accountID, email }, async () => {
                    ...
                    const workspaces = await Database.use((tx) => tx
